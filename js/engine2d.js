...
function generateStress(){
    ...
    document.getElementById("inputData").value=output.join(" ");
}
...
function renderImage(){
    ...
    if(format==="colorcode"){
        ...
        if(values.length !== w*h){
            alert("ColorCode requires exactly X*Y values.");
            return;
        }
        ...
        if(values.length !== w*h*3){
            alert("Numeric formats require X*Y*3 values.");
            return;
        }
        ...
        for(let i=0;i<w*h;i++){
            ...
            if(r===null||g===null||b===null){
                alert("Invalid numeric detected.");
                return;
            }
